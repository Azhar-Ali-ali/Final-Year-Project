...
files = [
    ...
    root / 'Front_End' / 'Admin' / 'js' / 'Payment-Payout-Management.js',
...
for path in files:
    ...
    text = text.replace("'http://localhost:5000/api/admin/reports'", "`${window.location.origin}/api/admin/reports`")
    text = text.replace("'http://localhost:5000'", "window.location.origin")
    text = text.replace("'http://127.0.0.1:5000'", "window.location.origin")

    if text != original:
        path.write_text(text, encoding='utf-8')

print('Updated API path prefixes in the frontend files.')
